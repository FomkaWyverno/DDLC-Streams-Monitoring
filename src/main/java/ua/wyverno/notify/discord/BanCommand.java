...
public class BanCommand extends ListenerAdapter {
    ...
    public BanCommand(TwitchService twitchService) {
    ...
    public void onSlashCommandInteraction(@NotNull SlashCommandInteractionEvent event) {
        if (event.getName().equals("ban")) {
            ...
            boolean addedBan = listBanStream
                    .add(new BanStream(userTwitch.getId(), userTwitch.getDisplayName()));

            String urlStream = "https://www.twitch.tv/"+userTwitch.getDisplayName();
            if (addedBan) {
                event.getHook().sendMessage("Успішно додано до списку прихованих \""
                        + userTwitch.getDisplayName() +"\"!\n"+urlStream).queue();
                logger.trace("Add to Ban Twitch Stream for user: {}\nURL: {}", userTwitch.getDisplayName(), urlStream);
            } else {
